...
final class Sink[-In, Mat](m: StreamLayout.Module, p: StreamLayout.InPort) {
  ...
  private[stream] def prepend[In2, Mat2](flow: Flow[In2, In, Mat2]): Sink[In2, Mat2] = {
    val flowCopy = flow.module.carbonCopy()

    new Sink(module
      .grow(flowCopy.module, (m1: Mat, m2: Mat2) ⇒ m2)
      .connect(flowCopy.outPorts(flow.forwardPort), backwardPort),
      flowCopy.inPorts(flow.backwardPort))
  }
}
...
object Sink {
   ...
   */
  def apply[T](props: Props): Sink[T, ActorRef] = new Sink(PropsSink(props))
   ...
   * the stream..
   */
  def foreach[T](f: T ⇒ Unit): Sink[T, Future[Unit]] = new Sink(new ForeachSink(f))
  ...
  def fold[U, T](zero: U)(f: (U, T) ⇒ U): Sink[T, Future[U]] = {
    ...
    Sink.ignore.prepend(Flow[T].section(name("fold")) { section ⇒
   ...
   */
  def onComplete[T](callback: Try[Unit] ⇒ Unit): Sink[T, Unit] = new Sink(new OnCompleteSink[T](callback))
}