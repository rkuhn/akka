...
case class Props[T](creator: () => Actor[T], deploy: Deploy)(implicit val tag: ClassTag[T]) {
...
object Props {
  def apply[T: ClassTag](block: => Actor[T]): Props[T] = Props(() => block, akka.actor.Props.defaultDeploy)
  ...
  private[typed] def apply[T](p: akka.actor.Props): Props[T] = {
    ...
    p.args match {
      case (creator: Function0[Actor[T]]) :: (tag: ClassTag[T]) :: Nil =>
