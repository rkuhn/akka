...
object Receptionist {
   ...
   */
  case class Register[T](key: ServiceKey[T], address: ActorRef[T])(val replyTo: ActorRef[Registered[T]]) extends Command
  /**
   * Query the Receptionist for a list of all Actors implementing the given
   * protocol.
   */
  case class Find[T](key: ServiceKey[T])(val replyTo: ActorRef[Listing[T]]) extends Command
